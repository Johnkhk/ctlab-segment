...
def color_splash(image, mask):
    ...
    if mask.shape[-1] > 0:
        ...
        splash = np.where(np.logical_not(mask), red, image).astype(np.uint8)
...
def dicom_folder_splash(model, folder_path=None):
    ...
    if folder_path:
        for im in os.listdir(folder_path):
            ...
            # output = output_folder + im +"_png_{:%Y%m%dT%H%M%S}.png".format(datetime.datetime.now())
            # cv2.imwrite(output, image) # Saving the image as png
            # image = skimage.io.imread(output)
